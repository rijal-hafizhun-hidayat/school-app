...
const { data, error } = await useApi<SchoolSubjectFetch>(
  `school-subject/${route.params.schoolSubjectId}`
);
if (data.value) {
  const schoolSubject: SchoolSubject = data.value.data as SchoolSubject;
  form.name = schoolSubject.name;
} else {
